package GenomeImporter::GenomeImporterClient;
...
GenomeImporter::GenomeImporterClient
...
sub new
{
    ...
    my $self = {
	client => GenomeImporter::GenomeImporterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
# Authentication: required
	...
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function import_external_genome (received $n, expecting 1)");
    }
    {
	my($params) = @args;
	...
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method import_external_genome",
...
sub version {
    ...
    if ($result) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(
            error => "Error invoking method import_external_genome",
            status_line => $self->{client}->status_line,
            method_name => 'import_external_genome',
        );
    }
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for GenomeImporter::GenomeImporterClient\n";
    }
    if ($sMajor == 0) {
        warn "GenomeImporter::GenomeImporterClient version is $svr_version. API subject to change.\n";
    }
}
...
package GenomeImporter::GenomeImporterClient::RpcClient;
