package GenomeImporter::GenomeImporterServer;
...
our %return_counts = (
        'import_external_genome' => 1,
        'status' => 1,
);

our %method_authentication = (
        'import_external_genome' => 'required',
);
...
sub call_method {
    ...
    my $ctx = GenomeImporter::GenomeImporterServerContext->new($self->{loggers}->{userlog},
...
{
    # Service GenomeImporter requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for GenomeImporter but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = GenomeImporter::GenomeImporterServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service GenomeImporter.");
    }
...
package GenomeImporter::GenomeImporterServerContext;

use strict;

=head1 NAME

GenomeImporter::GenomeImporterServerContext
...
package GenomeImporter::GenomeImporterServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = GenomeImporter::GenomeImporterServer->new(
